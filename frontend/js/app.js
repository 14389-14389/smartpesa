...
async function loadDashboard() {
    ...
    try {
        ...
        const transactions = await fetchWithAuth(
        ...
        // Load risk score
        try {
            const risk = await fetchWithAuth(
                `${API_BASE}/forecast/${currentBusinessId}/risk-alert`
            );
            const riskData = await risk.json();
            if ($('risk-score')) $('risk-score').textContent = riskData.risk_score || 0;
        } catch (e) {
                ...
                grid.innerHTML = suppliers.map(s => `
                    <div class="supplier-card">
                        ...
                        <div class="supplier-footer">
                            <span class="payment-badge">${s.payment_terms || 'Terms not set'}</span>
    ...
    try {
        ...
        if (tbody) {
            if (!payments.length) {
                ...
                // Get supplier names
                const suppliersRes = await fetchWithAuth(
                    `${API_BASE}/suppliers/?business_id=${currentBusinessId}`
                );
...
async function loadRiskAnalysis() {
    ...
    try {
        ...
        const forecastRes = await fetchWithAuth(
            `${API_BASE}/forecast/${currentBusinessId}/30days`
        );
...
window.deletePayment = async function(paymentId) {
    ...
    try {
        const response = await fetch(`${API_BASE}/suppliers/payments/${paymentId}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${authToken}` }
        });
...
window.markPaymentPaid = async function(paymentId) {
    ...
    try {
        const response = await fetch(`${API_BASE}/suppliers/payments/${paymentId}/pay`, {
            method: 'PUT',
            headers: { 'Authorization': `Bearer ${authToken}` }
        });
